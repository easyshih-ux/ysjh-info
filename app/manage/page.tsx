...
function EditorExtras({ item, setItem }: { item: Announcement; setItem: (item: Announcement) => void }) {
  ...
  return <div className={styles.extras}>
    <section><h3>重要日期／活動</h3>{item.importantEvents.map((entry, index) => <div className={styles.eventRow} key={index}><label>開始日期<Input type="date" value={entry.date} onChange={event => updateEvent(index, "date", event.target.value)} /></label><label>開始時間（選填）<Input type="time" value={entry.time ?? ""} onChange={event => updateEvent(index, "time", event.target.value)} /></label><label>結束日期（選填）<Input type="date" min={entry.date || undefined} value={entry.endDate ?? ""} onChange={event => updateEvent(index, "endDate", event.target.value)} /></label><label>結束時間（選填）<Input type="time" value={entry.endTime ?? ""} onChange={event => updateEvent(index, "endTime", event.target.value)} /></label><label>事項名稱<Input value={entry.title} onChange={event => updateEvent(index, "title", event.target.value)} /></label><Button aria-label="移除重要事項" variant="ghost" size="icon" onClick={() => setItem({ ...item, importantEvents: item.importantEvents.filter((_, itemIndex) => itemIndex !== index) })}><Trash2 /></Button></div>)}<Button variant="outline" size="sm" onClick={() => setItem({ ...item, importantEvents: [...item.importantEvents, { date: "", title: "" }] })}>＋ 新增重要事項</Button></section>
