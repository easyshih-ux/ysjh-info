...
import { AUDIENCES, formatFollowUpType, formatImportantEventSchedule, toggleAudienceSelection, type Announcement, type Audience, type FollowUp } from "@/lib/announcements";
import { CURRENT_ACADEMIC_YEAR, FRONTEND_ACADEMIC_YEARS } from "@/lib/academicYear";
import { DEPARTMENTS, MAX_CUSTOM_DEPARTMENT_LENGTH, OTHER_DEPARTMENT_OPTION, isDepartment, type Department } from "@/lib/departments";
...
export default function ManagePage() {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (!original) throw new AnnouncementManagementError("找不到要修改的公告，請重新整理後再試一次。");
      const updatedAt = await updateManagedAnnouncement(original, editing);
      if (updatedAt) setItems(current => current.map(item => item.id === editing.id ? applyAnnouncementUpdate(item, editing, updatedAt) : item));
      setEditing(null); setNotice(updatedAt ? "公告已更新" : "公告內容沒有變更");
    } catch (caught) {
  ...
  return <main className={styles.page}>
    ...
    <Dialog open={!!editing} onOpenChange={open => { if (!open && !saving) { setEditing(null); setError(""); } }}><DialogContent className={styles.editor} showCloseButton={!saving}>{editing && <>
      ...
      <ContactEditor item={editing} setItem={setEditing} />
...
function ContactEditor({ item, setItem }: { item: Announcement; setItem: (item: Announcement) => void }) {
...
function AnnouncementDetails({ item, publisher, onClose }: { item: Announcement | null; publisher: AuthorizedPublisherContextValue; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    readAnnouncementFollowUps(item.id).then(value => { if (active) setCurrentFollowUps(value); }).catch(() => undefined).finally(() => { if (active) setFollowUpsLoading(false); });
    return () => { active = false; };
  }, [item?.id]);
