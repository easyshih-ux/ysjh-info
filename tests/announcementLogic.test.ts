...
import { ACADEMIC_YEAR_STORAGE_KEY, announcementTimeStates, announcementsForIdentity, chasingAnnouncements, daysUntil, deadlineDateLabel, deadlineRelativeLabel, deadlineUrgency, importantEventDateLabel, isForIdentity, localDate, needsAcademicYearConfirmation, nextWeekEvents, readSavedIdentity, ROLE_STORAGE_KEY, saveIdentity, upcomingDeadlines, filterAnnouncements, weekBounds, weeklyEvents } from "../lib/announcementLogic.ts";
...
test("學校週次固定星期日至星期六",()=>{const {start,end}=weekBounds(new Date("2026-09-23T10:00:00+08:00"));assert.equal(start.getDay(),0);assert.equal(end.getDay(),6);assert.equal(start.getDate(),20);assert.equal(end.getDate(),26)});
test("跨週活動會在兩週持續列為本週重點",()=>{const item={...mockAnnouncements[0],importantEvents:[{date:"2026-09-25",endDate:"2026-10-02",title:"跨週活動"}]};assert.equal(weeklyEvents([item],new Date("2026-09-23T10:00:00+08:00")).length,1);assert.equal(weeklyEvents([item],new Date("2026-09-28T10:00:00+08:00")).length,1)});
test("下週才開始的事項進入下週預告並於下一週轉為本週",()=>{const item={...mockAnnouncements[0],importantEvents:[{date:"2026-10-01",title:"下週事項"}]};assert.equal(nextWeekEvents([item],new Date("2026-09-23T10:00:00+08:00")).length,1);assert.equal(weeklyEvents([item],new Date("2026-10-01T10:00:00+08:00")).length,1);assert.equal(nextWeekEvents([item],new Date("2026-10-01T10:00:00+08:00")).length,0)});
test("已開始跨週事項不重複進下週預告，催繳公告可優先分離",()=>{const item={...mockAnnouncements[0],collectionStatus:"chasing" as const,importantEvents:[{date:"2026-09-25",endDate:"2026-10-02",title:"跨週活動"}]};assert.equal(nextWeekEvents([item],new Date("2026-09-23T10:00:00+08:00")).length,0);assert.deepEqual(chasingAnnouncements([item]).map(value=>value.id),[item.id])});
test("發布、時間與催繳狀態維持獨立計算",()=>{const item={...mockAnnouncements[0],publicationStatus:"published" as const,collectionStatus:"chasing" as const,importantEvents:[{date:"2026-09-20",endDate:"2026-09-25",title:"活動"}],deadlines:[{date:"2026-09-24",label:"截止"}]};assert.deepEqual(announcementTimeStates(item,new Date("2026-09-23T10:00:00+08:00")),{deadline:"即將截止",activity:"活動進行中"});assert.equal(item.publicationStatus,"published");assert.equal(item.collectionStatus,"chasing")});
