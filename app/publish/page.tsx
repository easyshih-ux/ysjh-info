...
import { AUDIENCES, toggleAudienceSelection, type Announcement, type Audience } from "@/lib/announcements";
...
function PublishForm() {
  ...
  const updateImportantEvent = (index: number, field: "date" | "endDate" | "time" | "title", value: string) => setDraft(current => ({ ...current, importantEvents: current.importantEvents.map((item, itemIndex) => itemIndex === index ? { ...item, [field]: value } : item) }));
  ...
  return <main className={styles.page}>
    ...
    <form className={styles.form} onSubmit={submit} noValidate>
      ...
      <section className={`${styles.section} ${styles.optionalSection}`} aria-labelledby="optional-title">
        ...
        <div className={styles.fixedEditor}><div className={styles.editorIntro}><h3>📌 重要日期／活動（選填）</h3><p>哪一天有活動、會議或事情要發生？</p><small>例如：9/20 07:52 晨讀公播、9/26 18:30 家長日</small></div>{draft.importantEvents.map((item, index) => <article key={index} className={styles.editorCard}><strong>重要事項{["①","②","③"][index]}</strong><div className={styles.threeFields}><label>開始日期<Input type="date" value={item.date} onChange={event => updateImportantEvent(index, "date", event.target.value)} /></label><label>結束日期（選填）<Input type="date" min={item.date || undefined} value={item.endDate ?? ""} onChange={event => updateImportantEvent(index, "endDate", event.target.value)} /></label><label>時間（選填）<Input type="time" value={item.time ?? ""} onChange={event => updateImportantEvent(index, "time", event.target.value)} /></label><label>事項名稱<Input value={item.title} onChange={event => updateImportantEvent(index, "title", event.target.value)} placeholder="例如：晨讀公播" /></label></div></article>)}{errors.importantEvents && <small className={styles.error}>{errors.importantEvents}</small>}</div>
