...
import { manageAnnouncementLifecycleHandler } from "../src/index.ts";
...
function store(initial: Record<string, Data>) {
  ...
  const firestore = {
    collection(name: string) { return { doc(id: string) { return { path: `${name}/${id}` }; } }; },
...
test("永久刪除僅 systemAdmin", async () => {
  ...
  await denied(manageAnnouncementLifecycleHandler(request("owner", "delete"), publisherStore.firestore));
  ...
  await manageAnnouncementLifecycleHandler(request("admin", "delete"), adminStore.firestore);
