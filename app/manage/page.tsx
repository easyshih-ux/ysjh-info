...
export default function ManagePage() {
  ...
  return <main className={styles.page}>
    ...
    <Dialog open={!!editing} onOpenChange={open => { if (!open && !saving) { setEditing(null); setError(""); } }}><DialogContent className={styles.editor} showCloseButton={!saving}>{editing && <>
      ...
      <fieldset><legend>適用對象</legend><div className={styles.audiences}>{AUDIENCES.map(value => <label key={value}><Checkbox checked={editing.audiences.includes(value)} onCheckedChange={checked => setEditing({ ...editing, audiences: checked ? [...editing.audiences, value] : editing.audiences.filter(item => item !== value) })} />{value}</label>)}</div></fieldset>
...
function EditorExtras({ item, setItem }: { item: Announcement; setItem: (item: Announcement) => void }) {
  const updateEvent = (index: number, field: "date" | "time" | "title", value: string) => setItem({ ...item, importantEvents: item.importantEvents.map((entry, entryIndex) => entryIndex === index ? { ...entry, [field]: value } : entry) });
  ...
  return <div className={styles.extras}>
    <section><h3>重要日期／活動</h3>{item.importantEvents.map((entry, index) => <div className={styles.row} key={index}><Input type="date" value={entry.date} onChange={event => updateEvent(index, "date", event.target.value)} /><Input type="time" value={entry.time ?? ""} onChange={event => updateEvent(index, "time", event.target.value)} /><Input value={entry.title} onChange={event => updateEvent(index, "title", event.target.value)} /><Button aria-label="移除重要事項" variant="ghost" size="icon" onClick={() => setItem({ ...item, importantEvents: item.importantEvents.filter((_, itemIndex) => itemIndex !== index) })}><Trash2 /></Button></div>)}<Button variant="outline" size="sm" onClick={() => setItem({ ...item, importantEvents: [...item.importantEvents, { date: "", title: "" }] })}>＋ 新增重要事項</Button></section>
